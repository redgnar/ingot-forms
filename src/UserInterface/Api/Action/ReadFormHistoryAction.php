...
/**
 * Every accepted save of a form, oldest first — enough to choose one, and
 * nothing more.
 */
final class ReadFormHistoryAction
{
    ...
    #[OA\Get(
        ...
        description: 'Oldest first. The documents themselves are read one at a time (`GET /api/forms/{id}/history/{seq}`), because a list carrying every version of every answer is a response nobody asked for. `confirmed` marks the save a confirmation locked — derived, never stored: confirming writes no values, so it is no revision of its own.',
    )]
    #[OA\Response(response: 200, description: 'The history, oldest first. Empty for a form nobody has filled in.', content: new OA\JsonContent(ref: '#/components/schemas/FormHistory'))]
