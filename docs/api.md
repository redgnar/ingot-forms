...
are asserted against real HTTP traffic by `tests/Http/OpenApiComplianceTest.php`, so
...
The definition is immutable after creation — changing it means delete and recreate. Problems inside the definition are reported with JSON Pointers rooted at `/definition`.
...
A form definition document. Its authoritative contract is the meta-schema at `src/Domain/Forms/form-definition.schema.json` (id, title, and 1–50 typed fields with unique names) — the constraints are deliberately not duplicated here.
