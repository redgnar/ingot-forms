...
function ownControls(scope) {
    return [...scope.querySelectorAll('[data-name][data-type]')].filter(
        (control) => listOwning(control, scope) === null && !unasked(control),
    );
}

function ownLists(scope) {
    return [...scope.querySelectorAll('[data-collection]')].filter(
        (list) => listOwning(list.parentElement, scope) === null && list.closest('template') === null && !unasked(list),
    );
}

// A question this document is not asking of these answers. Its answer is not
// collected, which is the whole point: the document has to match the contract
// rather than be refused by it — the server derives `{"properties": {"nip":
// false}}` from the same condition, so a value sent for an unasked question is
// not merely ignored but refused.
//
...
//
// One scope at a time, because a condition names an item declared beside it — so
// an entry's questions are answered by that entry's own answers, and the same
// list drawn three times decides three times over.
function ask(scope = document.getElementById('form')) {
    for (const list of ownLists(scope)) {
        for (const entry of entriesOf(list)) ask(entry);
    }

    const blocks = [...scope.querySelectorAll('[data-asked-when], [data-required-when]')].filter(
        (block) => listOwning(block.dataset.collection === undefined ? block : block.parentElement, scope) === null
            && block.closest('template') === null,
    );

    if (blocks.length === 0) return;

    // A chain settles in as many passes as it is long: a question may be asked
    // on the strength of an answer to a question that is not being asked, and an
...
function askOnce(scope, blocks) {
    ...
    for (const block of blocks) {
        ...
        // An item a client fills in was never to be looked at, so it stays out
        // of sight whether or not it is being asked for.
        block.hidden = !asked || block.dataset.outOfSight !== undefined;

        if (asked) delete block.dataset.unasked;
        else block.dataset.unasked = '';

        // A refusal about a question nobody is being asked is a message about
        // nothing, and it would stand there unanswerable.
        if (!asked) clearMessagesIn(block);

        const requiredWhen = block.dataset.requiredWhen;

        if (requiredWhen !== undefined) owe(block, holds(JSON.parse(requiredWhen), answers));
    }

    return moved;
}

// The star, and what it stands for: a page draws one for an answer that is owed,
// and a condition coming about is one of the ways an answer becomes owed. The
// star is for eyes, `aria-required` for everybody else — a page that shows one
// without the other has told half its readers.
function owe(block, owed) {
...
// What the last attempt said about one question, when the page stops asking it.
function clearMessagesIn(block) {
    for (const slot of block.querySelectorAll('[data-error]')) {
        slot.hidden = true;
        slot.textContent = '';
    }

    for (const control of block.querySelectorAll('[aria-invalid]')) {
...
// Which questions are asked follows what has been answered, so it is worked out
